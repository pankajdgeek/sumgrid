...
$CURSOR_FILE = Join-Path $REPO_ROOT '.cursor/rules/spec-flow-rules.mdc'
$QWEN_FILE = Join-Path $REPO_ROOT 'QWEN.md'
$AGENTS_FILE = Join-Path $REPO_ROOT 'AGENTS.md'
$WINDSURF_FILE = Join-Path $REPO_ROOT '.windsurf/rules/spec-flow-rules.md'
$KILOCODE_FILE = Join-Path $REPO_ROOT '.kilocode/rules/spec-flow-rules.md'
$AUGGIE_FILE = Join-Path $REPO_ROOT '.augment/rules/spec-flow-rules.md'
$ROO_FILE = Join-Path $REPO_ROOT '.roo/rules/spec-flow-rules.md'

$TEMPLATE_FILE = Join-Path $REPO_ROOT '.spec-flow/templates/agent-file-template.md'
...
function Validate-Environment {
    if (-not $CURRENT_BRANCH) {
        ...
        if ($HAS_GIT) { Write-Info "Make sure you're on a feature branch" } else { Write-Info 'Set SPEC_FLOW_FEATURE environment variable or create a feature first' }
        exit 1
    }
    if (-not (Test-Path $NEW_PLAN)) {
        Write-Err "No plan.md found at $NEW_PLAN"
        Write-Info 'Ensure you are working on a feature with a corresponding spec directory'
        if (-not $HAS_GIT) { Write-Info 'Use: $env:SPEC_FLOW_FEATURE=your-feature-name or create a new feature first' }
        exit 1
    }
    if (-not (Test-Path $TEMPLATE_FILE)) {
        Write-Err "Template file not found at $TEMPLATE_FILE"
        Write-Info 'Run spec-flow init to scaffold .spec-flow/templates, or add agent-file-template.md there.'
        exit 1
    }
}
